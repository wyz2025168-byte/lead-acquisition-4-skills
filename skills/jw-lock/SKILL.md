---
name: jw-lock
description: 按新版课程执行四锁判断，从已批准产品反推可成交人群、熟悉人群、买单前阶段与话题，并形成可追溯锁客基线。用于用户要明确目标客户、检验锁客方向或处理人群阶段漂移时。
---
...
回答“产品到底能成交谁，应该先做哪类人的生意？”，并严格按冻结合同生成 `lock_baseline`。保持原始输入、事实状态、来源、审批、版本和哈希；信息不足时只追问最小缺口。
...
1. 执行 `C10-D01`：由已批准产品反推可成交人群候选
2. 执行 `C10-D02`：检查正价成交事实，不伪造稳定天数、单量或金额阈值
3. 执行 `C10-D03`：锁定认为问题值得当前价格与额外成本的产品可成交人群
4. 执行 `C10-D04`：在可成交候选中锁定创始人真正熟悉其心理、纠结、疑问和表达的人
5. 执行 `C10-D05`：锁定目标人当前的买单前具体阶段，不把买单后交付阶段当获客默认内容
6. 执行 `C10-D06`：锁定买单前关心的价值、方向、生活或行业话题，排除默认展开的买单后解决方法
7. 执行 `C10-D07`：将四锁合并成唯一可向下传递的人群、阶段与话题边界
