---
name: jw-positioning
description: 编译新版课程的定位基座，诊断真实需求、竞争心智与创始人适配，并形成可追溯的定位基线。用于用户要判断业务方向、定位三角、产品边界，或上游定位变化需要重审时。
---
...
回答“这件事最能成功的方向和打法是什么？”，并严格按冻结合同生成 `positioning_baseline`。保持原始输入、事实状态、来源、审批、版本和哈希；信息不足时只追问最小缺口。
...
1. 执行 `C00-D01`：把定位首先视为消费者为什么选你的商业选择，不以流量问题替代。
2. 执行 `C00-D02`：判断线上客户真正关心、具有价值且值得进入的需求方向。
3. 执行 `C00-D03`：识别竞争者与替代选择已占据的心智，判断可选的独特购买理由/心智空位前提。
4. 执行 `C00-D04`：将创始人身份、经历、表达/表现、交付和资源条件纳入方向判断。
5. 执行 `C00-D05`：在需求、竞争心智与创始人自身条件的交集中形成候选并保留取舍理由。
